...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    console.log(`Chegou p/ logins: ${JSON.stringify(req.body)}`);
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    nomeCompleto = req.body.nomeCompleto; // depois de .body, use o nome (name) do campo em seu formulário de login
    ...
    if (nomeCompleto == undefined || telefone == undefined || email == undefined || senha == undefined) {
	  // coloque a frase de erro que quiser aqui. Ela vai aparecer no formulário de cadastro
      throw new Error(`Dados de cadastro não chegaram completos: ${nomeCompleto} / ${telefone} / ${email} / ${senha}`);
    }
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into cadastro (nomeCompleto, telefone, email, senha) values ('${nomeCompleto}','${telefone}','${email}', '${senha}')`).then(function() {
